...
void printDifferencesBetweenProcesses(ProcessData *parentData, ProcessData *childData) {
    Differences diffs = { 0 };
    printf("\n----- ROZNICE MIEDZY PROCESAMI ( funkcja fork() ) -----\n\n");
...
int main()
{
    ...
    switch (pid) {
        ...
        case 0:
            ...
            write(dataPipe[1], &processData, sizeof(processData));
            exit(EXIT_SUCCESS);
        default:
            close(dataPipe[1]);
            wait(NULL);
    ...
    ProcessData *childData = malloc(sizeof(*childData));
    read(dataPipe[0], childData, sizeof(*childData));
